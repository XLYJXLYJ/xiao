<template>
  <div class="contain">
    <ul class="log-list">
      <li v-for="(item,index) in futureData" :key="index">
...
export default {
  ...
  mounted(){
    ...
    db.collection('futureList').get({
      success(res) {
        // res.data 包含该记录的数据
        that.futureData = res.data  
      }
    })
  },
  methods:{
    previewImg(index){
      let This = this;
      let arr = []
      arr.push(This.futureData[index].src)
      wx.previewImage({
          current:This.futureData[index].src,
          urls:arr
      });
    },
  }
